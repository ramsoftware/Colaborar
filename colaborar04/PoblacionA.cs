...
namespace Comparacion {
    internal class PoblacionA {
		//Almacena los individuos de la población
		List<IndividuoA> Individuos = new List<IndividuoA>();

		//Inicializa la población con un número de individuos
		public PoblacionA(Random azar, int numIndividuos, int RangoCoeficientes) {
			Individuos.Clear();
			for (int cont = 1; cont <= numIndividuos; cont++)
				Individuos.Add(new IndividuoA(azar, RangoCoeficientes));
		}
		...
		public void Proceso(Random azar, List<double> Entradas, List<double> Salidas, int posInicial, int posFinal, long TiempoParaOperar) {
			...
			//Console.Write("Genético Tradicional. Tiempo: " + cronometro.ElapsedMilliseconds.ToString() + " Itera B: " + iteraB.ToString());
		}

		public void CopiaMuta(Random azar, int Origen, int Destino) {
			IndividuoA ganador, perdedor;
