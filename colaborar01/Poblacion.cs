...
namespace DividirDatos {
	class Poblacion {
		...
		public Poblacion(Random azar, int numIndividuos, int Rango) {
			for (int cont = 1; cont <= numIndividuos; cont++) Individuos.Add(new Individuo(azar, Rango));
		}

		public void Proceso(Random azar, List<double> Entradas, List<double> Salidas, int posInicial, int posFinal, int numCiclos) {
			//Busca el individuo con mejor ajuste
